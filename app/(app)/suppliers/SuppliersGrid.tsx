...
/**
 * WHAT A PERSON ARRIVES HOLDING: a company name off a call, a CAGE off an award, or a place they
 * are trying to source from. Deliberately not tier, score, employees or SAM status — those are
 * the tabs above, and "manufacturer" as a search term would return every stocking firm.
 *
 * Shared with the grid's own box: the SAME function is handed to `DataGrid.search.fields` and
 * used to count matches across the whole book for the cross-tab note, so the two can never
 * disagree about what "matches" means.
 */
export const SEARCH_FIELDS = (r: LeanSupplier) => [r.company, r.cage, r.city, r.state];
...
export function SuppliersGrid({
  ...
  /*
   * THE QUERY IS OWNED BY THE GRID, AND MIRRORED HERE.
   *
   * `DataGrid` renders the box and does the filtering, because three other pages needed the same
   * thing and four hand-rolled boxes is four definitions of what a second search word means. What
   * the grid CANNOT know is that this page shows a slice of a larger book, so it reports the query
   * back and this component answers the question the grid cannot: is the firm somewhere else.
   */
  ...
  /*
   * The page's OWN filters only. The search is applied inside `DataGrid`, so `shown` is the
   * population the query is applied TO, not the result of it — which is exactly what the
   * cross-tab count below needs, and what the grid needs as its denominator.
   */
  const shown = useMemo(() => {
  ...
  /* How many rows survive the query inside the current tab. Reported by the grid, because the
     grid is what filtered them, and a second computation here would be a second opinion. */
  const [matchedHere, setMatchedHere] = useState<number | null>(null);
  const onQueryChange = useCallback((q: string, matched: number) => {
    setQuery(q);
    setMatchedHere(matched);
  }, []);
   ...
   * in the book" - a false statement about the data made by a UI decision.
   *
   * So when the query finds nothing HERE, it is counted across the whole book and the page says
   * which of the two happened. The tab is never switched silently: being moved out of the filter
   * you chose is its own small betrayal. It is offered as one press.
   */
  const elsewhere = useMemo(() => {
    if (terms.length === 0 || matchedHere !== 0) return 0;
    return suppliers.reduce((n, r) => (matchesTerms(haystackOf(SEARCH_FIELDS(r)), terms) ? n + 1 : n), 0);
  }, [suppliers, terms, matchedHere]);
    ...
    <>
      ...
      <div className={styles.toolbar}>
        ...
        <div className={styles.toolbarRight}>
          <span className={styles.resultCount} aria-live="polite">
            {terms.length > 0 && matchedHere !== null
              ? `${matchedHere.toLocaleString()} matching "${query.trim()}" of ${suppliers.length.toLocaleString()} in the book`
      ...
      <DataGrid
        ...
        search={{
          ...
          label: "Search the supplier book by company, CAGE code, city or state",
          onQueryChange,
        }}
        density="compact"
        empty={{
           ...
           */
          /* The grid replaces this with its own sentence when the SEARCH is what emptied it,
             so this one only ever speaks for the tabs. */
