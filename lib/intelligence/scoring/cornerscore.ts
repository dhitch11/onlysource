...
export function scoreCorner(
  ...
  // The lockup verdict, rendered as a first-class reason: the −40 penalty when locked, or the
  ...
  reasons.push({
    leg: "lockup",
    plain: lockup.plain,
    points: isLocked ? -40 : lockup.openDoorPoints > lastAwardeeDoor ? lockup.openDoorPoints : 0,
   ...
   *   rankKey = 10 (demand floor) + valuePoints(v) + waynePoints + min(30, cornerBucket) − 40·locked
  ...
  // The lock penalty MUST dominate the maximum possible positive score, not merely offset it: value
  // is uncapped, so a fixed −40 let a high-value locked row (e.g. a $10M part → 53 value points)
  // still land at a positive rankKey and outrank open rows. Every non-locked row is ≥ 10 (the demand
  // floor), so a penalty larger than any reachable score guarantees a locked row can never outrank an
  // open one — the "not even in sight" requirement enforced by the SCORE, not by the display filter
  // alone. Locked rows still order among themselves by their (value + wayne) tiebreak, all far below
  // every shown row, and their clamped scoreV0 correctly reads 0.
  const LOCK_PENALTY = 1000;
