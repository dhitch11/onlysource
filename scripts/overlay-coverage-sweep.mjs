...
const PROBE = (AREA_FAIL_RATIO) => {
  ...
  for (const c of controls) {
    ...
    for (const o of overlays) {
      ...
      if (centreBlocked || ratio >= AREA_FAIL_RATIO) {
        findings.push({
          kind: kindOf(o), control: describe(c), overlay: describe(o),
...
async function selftest() {
  ...
  for (const [file, expect] of [['covered.html', true], ['clear.html', false]]) {
    ...
    const r = await p.evaluate(PROBE, AREA_FAIL_RATIO)
...
for (const width of WIDTHS) {
  ...
  for (const route of routes) {
    ...
    const r = await page.evaluate(PROBE, AREA_FAIL_RATIO)
    visited++
    const bad = r.findings.filter((f) => f.kind === 'fixed' && (f.centreBlocked || f.ratio >= AREA_FAIL_RATIO * 100))
    ...
    if (bad.length) {
      ...
      for (const f of bad.slice(0, 4)) {
        console.log(`        ${f.centreBlocked ? 'CENTRE BLOCKED' : `${f.ratio}% covered`} (${f.px}px)  ${f.overlay}  over  ${f.control}`)
      }
